...
int main(int argc, char** argv)
{
 ...
 unsigned short PQA[ncols], RADSAT[ncols];
 unsigned char AEQA[ncols];
...
for(i=0;i<nrows;i++){
    ...
    fread(RADSAT,ncols,sizeof(short),fQA[1]);
    ...
    if((BLU[j]!=NoData)&&(RED[j]!=NoData)&&(NIR[j]!=NoData)&&(SWIR1[j]!=NoData)&&(SWIR2[j]!=NoData)){
        ...
        Decode_PQA(RADSAT[j],RADSAT_bits,16);
        Decode_AEQA(AEQA[j],AEQA_bits,8);
        ...
        if((PQA_bits[0]==1)||(PQA_bits[2]==1)){
         ...
         if((RADSAT[j]==0)||(RADSAT_bits[2]!=1&&RADSAT_bits[4]!=1&&RADSAT_bits[5]!=1&&RADSAT_bits[6]!=1&&RADSAT_bits[7]!=1)){
           if((PQA_bits[1]==1)&&(AEQA_bits[1]==1)){
             VI_QA = 20; // best quality land pixels
           }else if((PQA_bits[1]==0)&&(AEQA_bits[1]==1)){
             VI_QA = 23; // cloud/snow contaminated land pixels with valid aerosol
           }else if((PQA_bits[1]==1)&&(AEQA_bits[1]==0&&AEQA_bits[5]==1)){
             VI_QA = 21; // clear but with interpolated aerosol land pixels
           }else if((PQA_bits[1]==1)&&(AEQA_bits[1]==0&&AEQA_bits[5]==0)){
             VI_QA = 22; // clear but with invalid aerosol land pixels
           }
