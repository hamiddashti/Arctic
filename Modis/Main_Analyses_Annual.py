...
lst_lulc = xr.open_dataarray(
	in_dir + "Natural_Variability_Annual_outputs/delta_lst_changed_lulc_component.nc"
)
lst_nv = xr.open_dataarray(
	in_dir + "Natural_Variability_Annual_outputs/delta_lst_changed_nv_component.nc"
)
lst_diff_total = xr.open_dataarray(
	in_dir + "Natural_Variability_Annual_outputs/delta_lst_total.nc"
)
...
EC = xr.open_dataarray(
	in_dir + "Data/Annual/Annual_ET/EC.nc"
)  # Vegetation transpiration
EC = EC.fillna(
	0
)  # fill nan values with zero to have correct sum later when calculating ET, ESW and ECI
EI = xr.open_dataarray(
	in_dir + "Data/Annual/Annual_ET/EI.nc"
)  # Interception from vegetation canopy
EI = EI.fillna(0)
ES = xr.open_dataarray(in_dir + "Data/Annual/Annual_ET/ES.nc")  # Soil evaporation
ES = ES.fillna(0)
EW = xr.open_dataarray(
	in_dir + "Data/Annual/Annual_ET/EW.nc"
)  # Water body, snow and ice evaporation. Penman evapotranspiration is regarded as actual evaporation for them.
EW = EW.fillna(0)

ET = EC + EI + ES + EW  # Total ET (sum of all components)
ECI = EC + EI  # canopy evapotranspiration
ESW = ES + EW  # soil/water/ice/snow evaporation


EC = EC.where(EC != 0)  # Convert back zeros to nan
EI = EI.where(EI != 0)
ES = ES.where(ES != 0)
EW = EW.where(EW != 0)
ET = ET.where(ET != 0)
...
def myboxplot_group(df1,df2,df3,columns,txt_pos,outname):
	...
	for tick, label in zip(pos, ax1.get_xticklabels()):
		ax1.text(
			...
			txt_pos,
			df1['name']+" = "+str(df1_mean[tick])+"$\pm$"+str(df1_sd[tick])+"\n" + 
			df2['name']+" = "+str(df2_mean[tick])+"$\pm$"+ str(df2_sd[tick])+"\n" +
			df3['name']+" = "+str(df3_mean[tick])+"$\pm$"+ str(df3_sd[tick])+"\n",
...
myboxplot_group(df1,df2,df3,columns=class_name,txt_pos=9,outname = "EF_sparse_EL_WL.png")
...
lst_EL_WL, albedo_EL_WL, et_EL_WL = extract_wl_el(
	orig_class=herb, val=5)  # There are enough data
...
myboxplot_group(df1,df2,df3,columns=class_name,txt_pos=9,outname = "Herb_sparse_EL_WL.png")
...
df_ec.boxplot(figsize=(16, 10))
plt.xticks(rotation=45, fontsize=16)
plt.ylim()
plt.title("EC", fontsize=20)
plt.ylabel("EC [mm $year^{-1}$]", fontsize=16)
plt.tight_layout()
plt.savefig(fig_dir + "EC_Boxplot.png")
plt.close()
...
df_es.boxplot(figsize=(16, 10))
plt.xticks(rotation=45, fontsize=16)
plt.title("ES", fontsize=20)
plt.ylabel("ES [mm $year^{-1}$]", fontsize=16)
plt.tight_layout()
plt.savefig(fig_dir + "ES_Boxplot.png")
plt.close()
...
df_ew.boxplot(figsize=(16, 10))
plt.xticks(rotation=45, fontsize=16)
plt.title("EW", fontsize=20)
plt.ylabel("EW [mm $year^{-1}$]", fontsize=16)
plt.tight_layout()
plt.savefig(fig_dir + "EW_Boxplot.png")
plt.close()
...
df_eci.boxplot(figsize=(16, 10))
plt.xticks(rotation=45, fontsize=16)
plt.title("ECI", fontsize=20)
plt.ylabel("ECI [mm $year^{-1}$]", fontsize=16)
plt.tight_layout()
plt.savefig(fig_dir + "ECI_Boxplot.png")
plt.close()
...
df_esw.boxplot(figsize=(16, 10))
plt.xticks(rotation=45, fontsize=16)
plt.title("ESW", fontsize=20)
plt.ylabel("ESW [mm $year^{-1}$]", fontsize=16)
plt.tight_layout()
plt.savefig(fig_dir + "ESW_Boxplot.png")
plt.close()
...
myboxplot_group(df1,df2,df3,columns=columns,txt_pos=200,outname = "ET_Components_integrated.png")
