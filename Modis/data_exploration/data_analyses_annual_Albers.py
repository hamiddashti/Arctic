...
for i in range(len(lc_names)):
    ...
    with open(out_dir + "results.txt", "a") as f:
        ...
        f.write(f"p-value intercept: {np.round(out[7][0],3)}\n")
        f.write(f"p-value slope: {np.round(out[7][1],3)}\n")
...
save(out_dir + "Annual_gain_loss_LST_Albers2.png")
...
for i in range(len(lc_names)):
    ...
    with open(out_dir + "results.txt", "a") as f:
        ...
        f.write(f"p-value intercept: {np.round(out[7][0],2)}\n")
        f.write(f"p-value slope: {np.round(out[7][1],2)}\n")
    ...
    for k in range(len(lc_names)):
        ...
        axs[axs_counter].text(-0.17,
                              ...
                              bbox=dict(boxstyle="larrow,pad=0.03",
                                        ...
                                        lw=2))
        axs[axs_counter].set_xlim(-1, 1)
        axs[axs_counter].set_ylim(-25, 25)
...
pltfont = {'fontname': 'Times New Roman'}
fig.supxlabel('Fractional change in land cover', fontsize=16, **pltfont)
fig.supylabel("$\Delta ET$", fontsize=16, **pltfont)
plt.tight_layout()

# This is figure (XXXX) in the manuscript
save(out_dir + "Annual_gain_loss_ET.png")

# --------------------------------------------------------------------------
with open(out_dir + "results.txt", "a") as f:
    f.write("\n-------------------------------------------------------\n\n")
    f.write("Analyzing the Albedo data\n")
    f.write("\n------------------------\n")
...
# This is figure (XXXX) in the manuscript
save(out_dir + "Annual_gain_loss_Albedo.png")
