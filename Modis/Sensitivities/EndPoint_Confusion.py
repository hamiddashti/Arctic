...
if analyses_mode == "Seasonal":
    lst = xr.open_dataarray(
        "/data/ABOVE/Final_data/LST_Final/LST/Seasonal_Mean/LST_Mean_MAM.nc")
    lst2003 = lst.loc['2003'].squeeze()
    lst2013 = lst.loc['2013'].squeeze()
    lst_lulc = xr.open_dataarray(out_dir +
                                 "Natural_Variability/Natural_Variability_" +
                                 analyses_mode + "_outputs/EndPoints/" +
                                 seasons[1] +
                                 "/delta_lst_changed_lulc_component_2013.nc")

    lst_nv = xr.open_dataarray(out_dir +
                               "Natural_Variability/Natural_Variability_" +
                               analyses_mode + "_outputs/EndPoints/" +
                               seasons[1] +
                               "/delta_lst_changed_nv_component_2013.nc")

    lst_diff_total = xr.open_dataarray(
        out_dir + "Natural_Variability/Natural_Variability_" + analyses_mode +
        "_outputs/EndPoints/" + seasons[1] + "/delta_lst_total_2013.nc")

    # albedo = xr.open_dataarray(in_dir + "ALBEDO_Final/Seasonal_Albedo/Albedo_Mean_"+seasons[1]+".nc")
    albedo = xr.open_dataarray(
        in_dir + "ALBEDO_Final/Seasonal_Albedo/Albedo_Mean_MAM.nc")
    ...
    EC = xr.open_dataarray(
        in_dir +
        "ET_Final/Seasonal_ET/EC_Mean_MAM.nc")  # Vegetation transpiration
    EI = xr.open_dataarray(
        in_dir +
        "ET_Final/Seasonal_ET/EI_Mean_MAM.nc")  # Vegetation transpiration
    ES = xr.open_dataarray(
        in_dir +
        "ET_Final/Seasonal_ET/ES_Mean_MAM.nc")  # Vegetation transpiration
    EW = xr.open_dataarray(
        in_dir +
        "ET_Final/Seasonal_ET/EW_Mean_MAM.nc")  # Vegetation transpiration
    ET = xr.open_dataarray(
        in_dir +
        "ET_Final/Seasonal_ET/ET_Mean_MAM.nc")  # Vegetation transpiration
    EC = EC.fillna(0)
    EI = EI.fillna(0)
    ES = ES.fillna(0)
    EW = EW.fillna(0)
