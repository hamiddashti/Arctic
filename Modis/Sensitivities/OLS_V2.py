...
in_dir = "/data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/Sensitivity/EndPoints/Seasonal/"
out_dir = "/data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/Sensitivity/EndPoints/Seasonal/"
...
lower_list = np.array([
    7, 14, 15, 21, 22, 23, 28, 29, 30, 31, 35, 36, 37, 38, 39, 40, 43, 44, 45,
    46, 47
])
...
ds = xr.open_dataset(in_dir + "Confusion_Table_MAM.nc")
...
for i in range(0, cnt_upper_final.shape[1] - 1):
    I = np.where(cnt_upper_final[:, i] > 75)
...
for k in range(1, len(lst_array)):
    lst_2003_array_df = pd.concat(
        [lst_2003_array_df, pd.DataFrame(lst_2003_array[k])],
        ignore_index=True,
        axis=1)
    lst_2013_array_df = pd.concat(
        [lst_2013_array_df, pd.DataFrame(lst_2013_array[k])],
        ignore_index=True,
        axis=1)
    lst_array_df = pd.concat(
        [lst_array_df, pd.DataFrame(lst_array[k])], ignore_index=True, axis=1)
    albedo_2003_array_df = pd.concat(
        [albedo_2003_array_df,
         pd.DataFrame(albedo_2003_array[k])],
        ignore_index=True,
        axis=1)
    albedo_2013_array_df = pd.concat(
        [albedo_2013_array_df,
         pd.DataFrame(albedo_2013_array[k])],
        ignore_index=True,
        axis=1)
    albedo_array_df = pd.concat(
        [albedo_array_df, pd.DataFrame(albedo_array[k])],
        ignore_index=True,
        axis=1)
    et_2003_array_df = pd.concat(
        [et_2003_array_df, pd.DataFrame(et_2003_array[k])],
        ignore_index=True,
        axis=1)
    et_2013_array_df = pd.concat(
        [et_2013_array_df, pd.DataFrame(et_2013_array[k])],
        ignore_index=True,
        axis=1)
    et_array_df = pd.concat(
        [et_array_df, pd.DataFrame(et_array[k])], ignore_index=True, axis=1)
    ec_2003_array_df = pd.concat(
        [ec_2003_array_df, pd.DataFrame(ec_2003_array[k])],
        ignore_index=True,
        axis=1)
    ec_2013_array_df = pd.concat(
        [ec_2013_array_df, pd.DataFrame(ec_2013_array[k])],
        ignore_index=True,
        axis=1)
    ec_array_df = pd.concat(
        [ec_array_df, pd.DataFrame(ec_array[k])], ignore_index=True, axis=1)
    eci_2003_array_df = pd.concat(
        [eci_2003_array_df, pd.DataFrame(eci_2003_array[k])],
        ignore_index=True,
        axis=1)
    eci_2013_array_df = pd.concat(
        [eci_2013_array_df, pd.DataFrame(eci_2013_array[k])],
        ignore_index=True,
        axis=1)
    eci_array_df = pd.concat(
        [eci_array_df, pd.DataFrame(eci_array[k])], ignore_index=True, axis=1)
    esw_2003_array_df = pd.concat(
        [esw_2003_array_df, pd.DataFrame(esw_2003_array[k])],
        ignore_index=True,
        axis=1)
    esw_2013_array_df = pd.concat(
        [esw_2013_array_df, pd.DataFrame(esw_2013_array[k])],
        ignore_index=True,
        axis=1)
    esw_array_df = pd.concat(
        [esw_array_df, pd.DataFrame(esw_array[k])], ignore_index=True, axis=1)
