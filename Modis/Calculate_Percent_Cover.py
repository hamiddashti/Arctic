...
in_dir = 'F:\\MYD21A2\\LULC\\'
out_dir = 'F:\\MYD21A2\\LULC\\PercentCover\\'
fname = 'ABoVE_LandCover_Simplified_Bh08v04.tif'
...
for i in np.arange(1,32):
    ...
    cpy_ds = gdal.Open('LST_Day_0804.tif')
    band = cpy_ds.GetRasterBand(1)
    cpy_mask = (band.ReadAsArray() == band.GetNoDataValue())

    
    outname = out_dir +str(years[i-1]) +'_'+'PercentCover'+'_'+fname  
    # Result raster, with same resolution and position as the copy raster
    dst_ds = drv.Create(outname, cpy_ds.RasterXSize, cpy_ds.RasterYSize,
