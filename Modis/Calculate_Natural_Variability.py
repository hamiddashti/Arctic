if __name__ == "__main__":
    ...
    def calculate_nv(LST, LUC, nband, years, win_size, dist_m, out_dir):
        ...
        for k in range(0, len(years) - 1):
			...
			* Here we use the concept of numpy stride_trick to create moving windows. 
            ...
            lst_val_not_changed_view = window_view(
                lst_val_not_changed, win_size, type="LST"
            )
            ...
            luc_val_not_changed_view = window_view(
                luc_val_not_changed, win_size, type="LULC"
            )
            ...
            for i in range(0, lst_val_changed_view.shape[0]):
                for j in range(0, lst_val_changed_view.shape[1]):
                    ...
                    delta_natural_variability[i, j] = np.nansum(
                        weighted_lst
                    ) / np.nansum(1 / dist_mask)
            ...
            delta_lst_changed.to_netcdf(
                out_dir + "delta_lst_changed_" + str(year2) + ".nc"
            )
            delta_lst_not_changed.to_netcdf(
                out_dir + "delta_lst_not_changed_" + str(year2) + ".nc"
            )
            delta_lst_nv.to_netcdf(
                out_dir + "delta_lst_changed_nv_component_" + str(year2) + ".nc"
            )
            delta_lst_lulc.to_netcdf(
                out_dir + "delta_lst_changed_lulc_component_" + str(year2) + ".nc"
            )

            delta_abs_luc_changed.to_netcdf(
                out_dir + "delta_abs_luc_changed_" + str(year2) + ".nc"
            )
            delta_abs_luc_not_changed.to_netcdf(
                out_dir + "delta_abs_luc_not_changed_" + str(year2) + ".nc"
            )
        ...
        delta_lst_total_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_lst_total], dim=years[1:]
        )
        ...
        delta_lst_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_lst_changed], dim=years[1:]
        )
        ...
        delta_lst_not_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_lst_not_changed], dim=years[1:]
        )
        ...
        delta_lst_changed_nv_component_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_lst_changed_nv_component],
            dim=years[1:],
        )
        ...
        delta_lst_changed_lulc_component_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_lst_changed_lulc_component],
            dim=years[1:],
        )
        ...
        delta_abs_luc_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_abs_luc_changed], dim=years[1:]
        )
        ...
        delta_abs_luc_not_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_abs_luc_not_changed],
            dim=years[1:],
        )
        ...
        delta_luc_loss_gain_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_luc_loss_gain_changed],
            dim=years[1:],
        )
        delta_luc_loss_gain_changed_concat = delta_luc_loss_gain_changed_concat.rename(
            {"concat_dim": "year"}
        )
        ...
        delta_luc_loss_gain_not_changed_concat = xr.concat(
            [xr.open_dataarray(f) for f in fname_delta_luc_loss_gain_not_changed],
            dim=years[1:],
        )
    ...
    if analyses_mode == "Monthly":
        ...
        def monthly_cal(k):
            ...
            LUC = xr.open_dataarray(LUC_dir + "LULC_2003_2014.nc")
            out_dir = (
                "/data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/Natural_Variability/Natural_Variability_Monthly_outputs/"
                + Months[k]
                + "/"
            )
            LST = xr.open_dataarray(in_dir + "LST_Mean_" + Months[k] + ".nc")
        ...
        print(
            "The monthly results are saved in /data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/Natural_Variability/Natural_Variability_Monthly_outputs/"
        )
        ...
        calculate_nv(LST, LUC, nband, years, win_size, dist_m, out_dir)
        print(
            "The growing season results are saved in /data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/Natural_Variability/Natural_Variability_Growing_outputs/"
        )
