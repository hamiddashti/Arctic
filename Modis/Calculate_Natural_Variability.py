...
def calculate_nv(LST, LUC, nband, years, win_size, dist_m):
	...
	delta_lst_not_changed_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_lst_not_changed], dim=years
	)
	...
	delta_lst_changed_nv_component_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_lst_changed_nv_component], dim=years
	)
	...
	delta_abs_luc_changed_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_abs_luc_changed], dim=years
	)
	...
	delta_abs_luc_not_changed_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_abs_luc_not_changed], dim=years
	)
	...
	delta_luc_loss_gain_changed_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_luc_loss_gain_changed], dim=years
	)
	...
	delta_luc_loss_gain_not_changed_concat = xr.concat(
		[xr.open_dataarray(f) for f in fname_delta_luc_loss_gain_not_changed], dim=years
	)
