...
# Location of the LST files and the associated QC
in_dir    = '/data/ABOVE/MODIS/MYD21A2/'
out_dir   = '/data/home/hamiddashti/mnt/nasa_above/working/modis_analyses/outputs/LST/'
DayLST    = 'LST_Day_1KM.nc'
DayQC     = 'QC_Day.nc'
NightLST  = 'LST_Night_1KM.nc'
NightQC   = 'QC_Night.nc'

print('\n--------- Filtering the LST data based on the quality flag) ----------\n')
t1 = time.time()
lst_day = xr.open_dataarray(in_dir+DayLST,chunks=({'lat': 2692, 'lon': 8089}))
lst_day_qc = xr.open_dataarray(in_dir+DayQC,chunks=({'lat': 2692, 'lon': 8089}))
lst_night = xr.open_dataarray(in_dir+NightLST,chunks=({'lat': 2692, 'lon': 8089}))
lst_night_qc = xr.open_dataarray(in_dir+NightQC,chunks=({'lat': 2692, 'lon': 8089}))
...
t2 = time.time()
print(f'\n--------- Filtering Done: {(t2-t1)/60} minutes ---------------------\n\n')
...
t2 = time.time()
print(f'\n--------- Growing season Done: {(t2-t1)/60} minutes\n\n')
...
t2 = time.time()
print(f'\n----------- Monthly mean Done: {(t2-t1)/60} minutes----------------------\n\n')
...
t2 = time.time()
print(f'\n----------- Group month Done: {(t2-t1)/60} minutes -----------------------\n\n')
...
print(f'\n----------- Seasonal mean Done: {(t2-t1)/60} minutes ---------------------\n\n')
