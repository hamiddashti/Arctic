...
def lst_luc_plot(dluc,dvar):
    ...
    for i in np.arange(0,10):
        ...
        p = np.poly1d(z)
        slope, intercept, r_value, p_value, std_err = stats.linregress(luc[idx],lst[idx])
        plt.scatter(luc,lst,color='lightgray')
        plt.plot(luc,p(luc),color=next(colors),label = class_nemes[i])

    plt.xlabel('Fractional change in land cover (2003-2014)',fontsize=fsize)
    plt.ylabel('Land Surface Temprature Change (2003-2014)',fontsize=fsize)
    plt.title('Avoslute LUC change vs. annual LST')
    plt.xlim(0, 100)
    plt.tight_layout()
    plt.legend(loc="center left", bbox_to_anchor=(1.0, 0.5))
    plt.subplots_adjust(right=0.82)
    plt.savefig(out_dir+'AnnualLST_vs_LUC.tif')
    plt.close()
