...
def modis_wget_generator(product, folder, tiles, dates, out_dir):
	...
	for i in np.arange(len(tiles)):
		for j in np.arange(len(modis_date)):
			tmp1 = (
				"wget --user=hamiddashti --password=Iran1140 -P "
				+ f_path[j]
				+ " -r -nd --no-parent -A"
			)
			tmp2 = (
	...
	name.insert(
		len(name) + 1, "echo " + "Downloading finished.Data stored in: " + out_dir
	)
...
def lai_filter(filepath, out, std):
	...
	if std == True:
		...
		outname_std_lai = filepath.replace(
			fname, fname.replace(".nc", "") + "_lai_dataset.nc"
		)
...
def lst_luc_plot(
	dluc, dvar, class_names, outname, mode, label, title, fsize, multiband, legend
):
	import matplotlib.pyplot as plt
	import numpy as np
	...
	if mode == "paper":
		...
		for i in np.arange(0, 6):
			...
			if multiband == True:
				plt.plot(
					luc, p(luc), color=next(colors), label=class_names[i], linewidth=2
				)
			else:
				plt.plot(luc, p(luc), color="r", label=class_names[i], linewidth=4)
		...
		plt.xlabel("Fractional change in land cover (2003-2013)", fontsize=fsize)
		plt.ylabel(label, fontsize=fsize)
		plt.title(title)
		# plt.xlim(0, 100)
		plt.tight_layout()
		if legend == True:
			plt.legend(loc="center left", bbox_to_anchor=(1.0, 0.5), fontsize=fsize)
